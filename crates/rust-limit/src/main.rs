...
fn main() -> ExitCode {
    ...
    if args.is_empty() {
        eprintln!("usage: cargo mvl-limit [--emit-verification-json] <FILE>...");
...
fn run_verification_mode(args: &[String]) -> ExitCode {
    ...
    for arg in args {
        ...
        match std::fs::read_to_string(&path) {
            Ok(source) => match lints::check_source(&source) {
                Ok(diagnostics) => {
    ...
    let report = build_check_report("rust-limit", current_timestamp(), records);
    println!(
        "{}",
