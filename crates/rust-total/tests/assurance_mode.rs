//! Integration tests for `cargo-mvl-total --emit-assurance-json` (spec
...
fn write_fixture(name: &str, content: &str) -> PathBuf {
    let dir =
        std::env::temp_dir().join(format!("rust-total-assurance-test-{}", std::process::id()));
...
fn assurance_mode_captures_a_read_error_as_a_diagnostic_instead_of_aborting() {
    let missing_path = std::env::temp_dir().join(format!(
        "rust-total-assurance-test-{}-nonexistent.rs",
        std::process::id()
    ));
