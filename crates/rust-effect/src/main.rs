...
fn main() -> ExitCode {
    ...
    // `cargo mvl-effect <paths>` re-invokes this binary as
    // `cargo-mvl-effect mvl-effect <paths>` — drop the re-inserted
    // subcommand name so direct invocation and `cargo mvl-effect` behave
    // identically.
    if args.first().map(String::as_str) == Some("mvl-effect") {
        args.remove(0);
    }
    ...
    if args.is_empty() {
        eprintln!("usage: cargo mvl-effect [--emit-verification-json] <FILE>...");
...
fn run_verification_mode(args: &[String]) -> ExitCode {
    ...
    let report = build_check_report("rust-effect", current_timestamp(), records);
    println!(
        "{}",
