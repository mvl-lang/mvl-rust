...
- **BREAKING** `rust-mcdc` (#121): obligation ids are now `<module-slug>_<fn>_<decision-hash>` instead of `<file-stem>_<line>`. The module slug (path after the last `src`, `mod.rs` collapsed to its directory) fixes cross-file collisions — `src/vm/batch.rs` and `src/codegen/batch.rs` used to share `batch_1359`, and `harvest` joins tags to obligations by id alone, so vectors were silently misattributed. The 8-hex FNV-1a hash of the whitespace-normalized decision text replaces the line number, so inserting or deleting code above a decision no longer retags it; the id changes exactly when the decision text changes. The enclosing `fn` name (innermost free function, `impl` method or trait default method; omitted outside any `fn`) makes tags readable and keeps the same condition in two functions apart; identical decisions in one `fn` get `_2`, `_3` suffixes in source order. `Decision` gains an `enclosing_fn: Option<String>` field. `Decision::to_record` is replaced by `scanner::to_records(file, &decisions)`, which assigns those suffixes. Every existing `mcdc__<id>__v<N>` tag changes id: consumers need a one-time `scan` snapshot regeneration and mechanical re-tag (by decision text) when bumping their pinned `MVL_RUST_REV`.
